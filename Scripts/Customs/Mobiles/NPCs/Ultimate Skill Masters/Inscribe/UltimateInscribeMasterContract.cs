...
namespace Server.Items
{
    public class UltimateInscribeMasterContract : Item
    {
		...
		public override void Deserialize(GenericReader reader)
		{
			...
			// repopulate the static list:
			InscribeCollectionType.PopulateInscribeCollection();

			string itemName = reader.ReadString();
			m_InscribeItem = InscribeCollectionType.Items
							  .FirstOrDefault(i => i.Name == itemName);

			// if still null, fall back to a random or default:
			if (m_InscribeItem == null)
				m_InscribeItem = InscribeCollectionType.GetRandomItem();
