...
namespace Server.Items
{
    ...
    public class CampingContract : Item
    {
        ...
        public CampingContract(PlayerMobile player) : base(0x14EF)
        {
            ...
            var validCreatures = CampingContractType.Get.Where(creatureType =>
            {
                try
                {
            ...
            {
                var selectedCreature = validCreatures[Utility.Random(validCreatures.Count)];
                CreatureType = Array.IndexOf(CampingContractType.Get, selectedCreature);
