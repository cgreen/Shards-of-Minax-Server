...
namespace Server.Items
{
    ...
    public class VeterinaryContract : Item
    {
        ...
        [Constructable]
        public VeterinaryContract(PlayerMobile player) : base(0x14EF)
        {
            Weight = 1.0;
            Movable = true;
            ...
            var candidates = VeterinaryContractType.Get
                .Where(ct =>
                {
                    try
                    {
            ...
            {
                Name = "Veterinary Contract: No tamable creatures available";
                return;
            }
            ...
            m_CreatureType = Array.IndexOf(VeterinaryContractType.Get, selected);
            ...
            Name = $"Veterinary Contract: {m_AmountToTame} {selected.Name}";

            // Power‐scroll defaults
            m_PowerScrollSkill = SkillName.Veterinary;
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            {
                // Calculate PowerScrollValue based on player's current Veterinary skill cap
                Skill Veterinary = from.Skills[SkillName.Veterinary];
                if (Veterinary != null)
                {
                    double currentCap = Veterinary.Cap;
                ...
                from.SendGump(new VeterinaryContractGump(from, this));
            }
            else
            {
        ...
        public VeterinaryContract(Serial serial) : base(serial) { }
