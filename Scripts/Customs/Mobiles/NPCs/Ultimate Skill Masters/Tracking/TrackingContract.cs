...
namespace Server.Items
{
    ...
    public class TrackingContract : Item
    {
        ...
        [Constructable]
        public TrackingContract(PlayerMobile player) : base(0x14EF)
        {
            Weight = 1.0;
            Movable = true;
            ...
            m_CreatureType = Array.IndexOf(TrackingContractType.Get, selected);
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            {
                // Calculate PowerScrollValue based on player's current Tracking skill cap
                Skill Tracking = from.Skills[SkillName.Tracking];
                if (Tracking != null)
                {
                    double currentCap = Tracking.Cap;
                ...
                from.SendGump(new TrackingContractGump(from, this));
            }
            else
            {
        ...
        public TrackingContract(Serial serial) : base(serial) { }
