...
namespace Server.Gumps
{
    public class DynamicVendorGump : Gump
    {
        ...
        private Mobile m_VendorMobile;
        private IDynamicVendor m_Vendor;
        ...
        public DynamicVendorGump(Mobile buyer, Mobile vendorMobile, IDynamicVendor vendor, int page = 0, int subPage = 0) : base(50, 50)
        {
            m_Buyer = buyer;
            m_VendorMobile = vendorMobile;
            m_Vendor = vendor;
            m_Page = page;
            m_SubPage = subPage;
        ...
        private void BuildGump()
        {
            ...
            AddLabel(180, 15, Hue_BrightWhite, $"--- {m_Vendor.VendorName} ---");
        ...
        {
            // Always check range against the vendor's Mobile, not the interface.
            if (!m_Buyer.CheckAlive() || m_VendorMobile == null || m_VendorMobile.Deleted || !m_Buyer.InRange(m_VendorMobile.Location, 8))
            {
                m_Buyer.SendLocalizedMessage(500446); // “You are too far away.”
                return;
            }
            ...
            {
                case 0: break;
                case 1: m_Buyer.SendGump(new DynamicVendorGump(m_Buyer, m_VendorMobile, m_Vendor, 0, 0)); break;
                case 2: m_Buyer.SendGump(new DynamicVendorGump(m_Buyer, m_VendorMobile, m_Vendor, 1, 0)); break;
                case 3:
                    if (m_SubPage > 0)
                        m_Buyer.SendGump(new DynamicVendorGump(m_Buyer, m_VendorMobile, m_Vendor, m_Page, m_SubPage - 1));
                    else
                        m_Buyer.SendGump(new DynamicVendorGump(m_Buyer, m_VendorMobile, m_Vendor, m_Page, m_SubPage));
                    break;
                case 4:
                    var listPageCheck = (m_Page == 0) ? m_Vendor.CurrentSellItems : m_Vendor.CurrentBuyItems;
                    int totalPgs = (listPageCheck.Count + ItemsPerPage - 1) / ItemsPerPage;
                    if (m_SubPage < totalPgs - 1)
                        m_Buyer.SendGump(new DynamicVendorGump(m_Buyer, m_VendorMobile, m_Vendor, m_Page, m_SubPage + 1));
                    else
                        m_Buyer.SendGump(new DynamicVendorGump(m_Buyer, m_VendorMobile, m_Vendor, m_Page, m_SubPage));
                    break;
                ...
                default: // Item action buttons
                    if (buttonId >= 1000 && buttonId < 2000) // Buy action
                    {
                        ...
                        if (index >= 0 && index < sellList.Count)
                        {
                            ...
                            if (m_Vendor.OnBuyItem(m_Buyer, itemType, price, quantity))
                                m_Buyer.SendGump(new DynamicVendorGump(m_Buyer, m_VendorMobile, m_Vendor, m_Page, m_SubPage)); // Refresh on success
                            else
                                m_Buyer.SendGump(new DynamicVendorGump(m_Buyer, m_VendorMobile, m_Vendor, m_Page, m_SubPage)); // Refresh on fail (to show errors/updated stock)
                        }
                        else
                        {
                            m_Buyer.SendGump(new DynamicVendorGump(m_Buyer, m_VendorMobile, m_Vendor, m_Page, m_SubPage)); // Refresh on index error
                        }
                    }
