...
namespace Server.Mobiles
{
    ...
    */
    ...
    public abstract class BaseDynamicVendor : BaseCreature, IDynamicVendor
    {
		...
		Mobile IDynamicVendor.Host
		{
			get { return this; }
		}

		public string VendorName => Name;

		Dictionary<Type, DynamicStockInfo> IDynamicVendor.CurrentSellItems => this.CurrentSellItems;
		Dictionary<Type, DynamicStockInfo> IDynamicVendor.CurrentBuyItems  => this.CurrentBuyItems;

		bool IDynamicVendor.OnBuyItem(Mobile buyer, Type itemType, int price, int quantity)
			=> this.OnBuyItem(buyer, itemType, price, quantity);

		void IDynamicVendor.StartSellItem(Mobile seller, Type itemType, DynamicStockInfo info)
			=> this.StartSellItem(seller, itemType, info);
        ...
        public override void OnSpeech(SpeechEventArgs e)
        {
            ...
            if (!e.Handled && from.InRange(this, 5)) // Increased range slightly for speech interaction
            {
                ...
                {
                    e.Handled = true;
                    from.SendGump( new DynamicVendorGump( from, this, this ) );
                }
         ...
         private class VendorBrowseEntry : ContextMenuEntry
        {
            ...
            public override void OnClick()
            {
                 ...
                 {
                      m_From.SendGump( new DynamicVendorGump( m_From, m_Vendor, m_Vendor ) );
                 }
            }
        }
