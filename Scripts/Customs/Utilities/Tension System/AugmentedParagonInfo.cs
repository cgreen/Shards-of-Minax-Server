    ...
    public static class ExtendedParagon
    {
        ...
        public static void ConvertToAugmentedParagon(BaseCreature bc)
        {
			...
			if (info.AbilityType != null)
			{
				...
				// Also attach XmlRandomTraits to give random trait-based behavior
				try
				{
					XmlRandomTraits traitsAttachment = new XmlRandomTraits();
					XmlDynamicVendor vendorAttachment = new XmlDynamicVendor();

					// Optionally customize traits file or range
					// XmlRandomTraits traitsAttachment = new XmlRandomTraits("path/to/traits.xml", 2, 5);

					XmlAttach.AttachTo(bc, traitsAttachment);
					XmlAttach.AttachTo(bc, vendorAttachment);
				}
				catch (Exception ex)
				{
					Console.WriteLine($"Error attaching XmlRandomTraits to {bc.Name}: {ex.Message}");
				}
