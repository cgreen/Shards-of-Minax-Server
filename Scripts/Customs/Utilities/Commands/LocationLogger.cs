...
namespace Server.Custom
{
    ...
    {
        private static readonly string FilePath = "LocationPoints.txt"; // File is saved in the ServUO root directory

        public static void Initialize()
        {
            CommandSystem.Register("AddLocationPoint", AccessLevel.Administrator, new CommandEventHandler(AddLocationPoint_OnCommand));
        }

        [Usage("AddLocationPoint")]
        [Description("Logs your current position to LocationPoints.txt in (x, y, z, Map.MapName) format.")]
        public static void AddLocationPoint_OnCommand(CommandEventArgs e)
        {
            ...
            string output = $"({location.X}, {location.Y}, {location.Z}, Map.{mapName}),";
            File.AppendAllText(FilePath, output + Environment.NewLine);

            from.SendMessage(0x40, $"Location logged: {output}");
        }
