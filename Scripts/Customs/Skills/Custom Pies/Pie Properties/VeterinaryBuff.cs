...
{
    public sealed class VeterinaryBuff : IPieAttribute
    {
        public string Id    { get { return "VeterinaryBuff"; } }
        public string Label { get { return "Veterinary Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Veterinary” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static VeterinaryBuff()
        {
            PieAttributeRegistry.Register(new VeterinaryBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Veterinary TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Veterinary” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Veterinary mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Veterinary)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Veterinary,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Veterinary for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
