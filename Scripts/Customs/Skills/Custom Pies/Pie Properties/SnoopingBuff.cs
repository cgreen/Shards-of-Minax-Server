...
{
    public sealed class SnoopingBuff : IPieAttribute
    {
        public string Id    { get { return "SnoopingBuff"; } }
        public string Label { get { return "Snooping Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Snooping” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static SnoopingBuff()
        {
            PieAttributeRegistry.Register(new SnoopingBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Snooping TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Snooping” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Snooping mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Snooping)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Snooping,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Snooping for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
