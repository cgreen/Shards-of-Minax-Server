...
{
    public sealed class NecromancyBuff : IPieAttribute
    {
        public string Id    { get { return "NecromancyBuff"; } }
        public string Label { get { return "Necromancy Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Necromancy” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static NecromancyBuff()
        {
            PieAttributeRegistry.Register(new NecromancyBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Necromancy TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Necromancy” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Necromancy mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Necromancy)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Necromancy,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Necromancy for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
