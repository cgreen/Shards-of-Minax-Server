...
{
    public sealed class FencingBuff : IPieAttribute
    {
        public string Id    { get { return "FencingBuff"; } }
        public string Label { get { return "Fencing Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Fencing” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static FencingBuff()
        {
            PieAttributeRegistry.Register(new FencingBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Fencing TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Fencing” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Fencing mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Fencing)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Fencing,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Fencing for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
