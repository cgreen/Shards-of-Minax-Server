...
{
    public sealed class CarpentryBuff : IPieAttribute
    {
        public string Id    { get { return "CarpentryBuff"; } }
        public string Label { get { return "Carpentry Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Carpentry” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static CarpentryBuff()
        {
            PieAttributeRegistry.Register(new CarpentryBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Carpentry TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Carpentry” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Carpentry mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Carpentry)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Carpentry,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Carpentry for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
