...
{
    public sealed class EvalIntBuff : IPieAttribute
    {
        public string Id    { get { return "EvalIntBuff"; } }
        public string Label { get { return "EvalInt Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “EvalInt” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static EvalIntBuff()
        {
            PieAttributeRegistry.Register(new EvalIntBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing EvalInt TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “EvalInt” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created EvalInt mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.EvalInt)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.EvalInt,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} EvalInt for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
