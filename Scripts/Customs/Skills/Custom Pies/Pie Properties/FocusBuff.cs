...
{
    public sealed class FocusBuff : IPieAttribute
    {
        public string Id    { get { return "FocusBuff"; } }
        public string Label { get { return "Focus Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Focus” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static FocusBuff()
        {
            PieAttributeRegistry.Register(new FocusBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Focus TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Focus” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Focus mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Focus)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Focus,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Focus for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
