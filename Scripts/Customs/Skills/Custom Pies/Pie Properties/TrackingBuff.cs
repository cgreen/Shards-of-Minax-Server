...
{
    public sealed class TrackingBuff : IPieAttribute
    {
        public string Id    { get { return "TrackingBuff"; } }
        public string Label { get { return "Tracking Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Tracking” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static TrackingBuff()
        {
            PieAttributeRegistry.Register(new TrackingBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Tracking TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Tracking” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Tracking mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Tracking)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Tracking,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Tracking for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
