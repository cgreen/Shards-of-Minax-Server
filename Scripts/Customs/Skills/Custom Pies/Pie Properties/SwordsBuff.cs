...
{
    public sealed class SwordsBuff : IPieAttribute
    {
        public string Id    { get { return "SwordsBuff"; } }
        public string Label { get { return "Swords Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Swords” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static SwordsBuff()
        {
            PieAttributeRegistry.Register(new SwordsBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Swords TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Swords” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Swords mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Swords)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Swords,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Swords for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
