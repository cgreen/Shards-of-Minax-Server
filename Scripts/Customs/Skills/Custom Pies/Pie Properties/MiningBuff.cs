...
{
    public sealed class MiningBuff : IPieAttribute
    {
        public string Id    { get { return "MiningBuff"; } }
        public string Label { get { return "Mining Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Mining” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static MiningBuff()
        {
            PieAttributeRegistry.Register(new MiningBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Mining TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Mining” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Mining mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Mining)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Mining,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Mining for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
