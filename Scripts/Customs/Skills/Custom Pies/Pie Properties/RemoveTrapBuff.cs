...
{
    public sealed class RemoveTrapBuff : IPieAttribute
    {
        public string Id    { get { return "RemoveTrapBuff"; } }
        public string Label { get { return "RemoveTrap Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “RemoveTrap” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static RemoveTrapBuff()
        {
            PieAttributeRegistry.Register(new RemoveTrapBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing RemoveTrap TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “RemoveTrap” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created RemoveTrap mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.RemoveTrap)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.RemoveTrap,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} RemoveTrap for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
