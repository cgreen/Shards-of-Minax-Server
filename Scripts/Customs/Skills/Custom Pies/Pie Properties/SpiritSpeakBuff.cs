...
{
    public sealed class SpiritSpeakBuff : IPieAttribute
    {
        public string Id    { get { return "SpiritSpeakBuff"; } }
        public string Label { get { return "SpiritSpeak Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “SpiritSpeak” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static SpiritSpeakBuff()
        {
            PieAttributeRegistry.Register(new SpiritSpeakBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing SpiritSpeak TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “SpiritSpeak” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created SpiritSpeak mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.SpiritSpeak)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.SpiritSpeak,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} SpiritSpeak for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
