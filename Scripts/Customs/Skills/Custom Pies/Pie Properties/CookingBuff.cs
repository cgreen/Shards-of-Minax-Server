...
{
    public sealed class CookingBuff : IPieAttribute
    {
        public string Id    { get { return "CookingBuff"; } }
        public string Label { get { return "Cooking Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Cooking” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static CookingBuff()
        {
            PieAttributeRegistry.Register(new CookingBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Cooking TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Cooking” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Cooking mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Cooking)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Cooking,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Cooking for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
