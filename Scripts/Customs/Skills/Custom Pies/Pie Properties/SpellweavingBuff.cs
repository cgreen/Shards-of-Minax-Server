...
{
    public sealed class SpellweavingBuff : IPieAttribute
    {
        public string Id    { get { return "SpellweavingBuff"; } }
        public string Label { get { return "Spellweaving Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Spellweaving” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static SpellweavingBuff()
        {
            PieAttributeRegistry.Register(new SpellweavingBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Spellweaving TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Spellweaving” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Spellweaving mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Spellweaving)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Spellweaving,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Spellweaving for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
