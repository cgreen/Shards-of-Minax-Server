...
{
    public sealed class LumberjackingBuff : IPieAttribute
    {
        public string Id    { get { return "LumberjackingBuff"; } }
        public string Label { get { return "Lumberjacking Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Lumberjacking” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static LumberjackingBuff()
        {
            PieAttributeRegistry.Register(new LumberjackingBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Lumberjacking TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Lumberjacking” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Lumberjacking mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Lumberjacking)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Lumberjacking,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Lumberjacking for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
