...
{
    public sealed class PoisoningBuff : IPieAttribute
    {
        public string Id    { get { return "PoisoningBuff"; } }
        public string Label { get { return "Poisoning Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Poisoning” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static PoisoningBuff()
        {
            PieAttributeRegistry.Register(new PoisoningBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Poisoning TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Poisoning” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Poisoning mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Poisoning)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Poisoning,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Poisoning for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
