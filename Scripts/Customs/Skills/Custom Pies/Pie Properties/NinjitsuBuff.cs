...
{
    public sealed class NinjitsuBuff : IPieAttribute
    {
        public string Id    { get { return "NinjitsuBuff"; } }
        public string Label { get { return "Ninjitsu Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Ninjitsu” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static NinjitsuBuff()
        {
            PieAttributeRegistry.Register(new NinjitsuBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Ninjitsu TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Ninjitsu” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Ninjitsu mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Ninjitsu)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Ninjitsu,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Ninjitsu for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
