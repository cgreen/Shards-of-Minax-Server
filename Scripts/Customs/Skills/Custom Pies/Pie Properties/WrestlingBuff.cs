...
{
    public sealed class WrestlingBuff : IPieAttribute
    {
        public string Id    { get { return "WrestlingBuff"; } }
        public string Label { get { return "Wrestling Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Wrestling” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static WrestlingBuff()
        {
            PieAttributeRegistry.Register(new WrestlingBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Wrestling TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Wrestling” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Wrestling mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Wrestling)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Wrestling,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Wrestling for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
