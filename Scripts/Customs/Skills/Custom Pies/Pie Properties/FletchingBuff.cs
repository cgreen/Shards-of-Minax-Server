...
{
    public sealed class FletchingBuff : IPieAttribute
    {
        public string Id    { get { return "FletchingBuff"; } }
        public string Label { get { return "Fletching Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Fletching” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static FletchingBuff()
        {
            PieAttributeRegistry.Register(new FletchingBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Fletching TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Fletching” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Fletching mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Fletching)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Fletching,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Fletching for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
