...
{
    public sealed class BlacksmithBuff : IPieAttribute
    {
        public string Id    { get { return "BlacksmithBuff"; } }
        public string Label { get { return "Blacksmith Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Blacksmith” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static BlacksmithBuff()
        {
            PieAttributeRegistry.Register(new BlacksmithBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Blacksmith TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Blacksmith” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Blacksmith mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Blacksmith)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Blacksmith,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Blacksmith for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
