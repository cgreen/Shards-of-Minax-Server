...
{
    public sealed class MysticismBuff : IPieAttribute
    {
        public string Id    { get { return "MysticismBuff"; } }
        public string Label { get { return "Mysticism Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Mysticism” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static MysticismBuff()
        {
            PieAttributeRegistry.Register(new MysticismBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Mysticism TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Mysticism” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Mysticism mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Mysticism)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Mysticism,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Mysticism for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
