...
{
    public sealed class AnimalLoreBuff : IPieAttribute
    {
        public string Id    { get { return "AnimalLoreBuff"; } }
        public string Label { get { return "AnimalLore Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “AnimalLore” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static AnimalLoreBuff()
        {
            PieAttributeRegistry.Register(new AnimalLoreBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing AnimalLore TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “AnimalLore” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created AnimalLore mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.AnimalLore)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.AnimalLore,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} AnimalLore for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
