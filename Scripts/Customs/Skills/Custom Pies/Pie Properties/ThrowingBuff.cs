...
{
    public sealed class ThrowingBuff : IPieAttribute
    {
        public string Id    { get { return "ThrowingBuff"; } }
        public string Label { get { return "Throwing Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Throwing” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static ThrowingBuff()
        {
            PieAttributeRegistry.Register(new ThrowingBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Throwing TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Throwing” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Throwing mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Throwing)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Throwing,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Throwing for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
