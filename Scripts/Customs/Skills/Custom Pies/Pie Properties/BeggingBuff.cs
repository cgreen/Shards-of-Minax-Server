...
{
    public sealed class BeggingBuff : IPieAttribute
    {
        public string Id    { get { return "BeggingBuff"; } }
        public string Label { get { return "Begging Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Begging” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static BeggingBuff()
        {
            PieAttributeRegistry.Register(new BeggingBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Begging TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Begging” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Begging mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Begging)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Begging,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Begging for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
