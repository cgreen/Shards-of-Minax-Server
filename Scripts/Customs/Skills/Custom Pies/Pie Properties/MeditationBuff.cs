...
{
    public sealed class MeditationBuff : IPieAttribute
    {
        public string Id    { get { return "MeditationBuff"; } }
        public string Label { get { return "Meditation Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Meditation” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static MeditationBuff()
        {
            PieAttributeRegistry.Register(new MeditationBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Meditation TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Meditation” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Meditation mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Meditation)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Meditation,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Meditation for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
