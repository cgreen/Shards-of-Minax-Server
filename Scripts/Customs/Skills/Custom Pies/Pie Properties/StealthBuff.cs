...
{
    public sealed class StealthBuff : IPieAttribute
    {
        public string Id    { get { return "StealthBuff"; } }
        public string Label { get { return "Stealth Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Stealth” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static StealthBuff()
        {
            PieAttributeRegistry.Register(new StealthBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Stealth TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Stealth” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Stealth mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Stealth)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Stealth,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Stealth for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
