...
{
    public sealed class AlchemyBuff : IPieAttribute
    {
        public string Id    { get { return "AlchemyBuff"; } }
        public string Label { get { return "Alchemy Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “alchemy” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static AlchemyBuff()
        {
            PieAttributeRegistry.Register(new AlchemyBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Alchemy TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Alchemy” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Alchemy mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Alchemy)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Alchemy,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Alchemy for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
