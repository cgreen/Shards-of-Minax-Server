...
{
    public sealed class TinkeringBuff : IPieAttribute
    {
        public string Id    { get { return "TinkeringBuff"; } }
        public string Label { get { return "Tinkering Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Tinkering” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static TinkeringBuff()
        {
            PieAttributeRegistry.Register(new TinkeringBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Tinkering TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Tinkering” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Tinkering mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Tinkering)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Tinkering,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Tinkering for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
