...
{
    public sealed class HidingBuff : IPieAttribute
    {
        public string Id    { get { return "HidingBuff"; } }
        public string Label { get { return "Hiding Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Hiding” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static HidingBuff()
        {
            PieAttributeRegistry.Register(new HidingBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Hiding TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Hiding” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Hiding mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Hiding)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Hiding,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Hiding for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
