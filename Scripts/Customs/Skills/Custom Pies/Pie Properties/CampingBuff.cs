...
{
    public sealed class CampingBuff : IPieAttribute
    {
        public string Id    { get { return "CampingBuff"; } }
        public string Label { get { return "Camping Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Camping” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static CampingBuff()
        {
            PieAttributeRegistry.Register(new CampingBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Camping TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Camping” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Camping mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Camping)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Camping,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Camping for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
