...
{
    public sealed class ImbuingBuff : IPieAttribute
    {
        public string Id    { get { return "ImbuingBuff"; } }
        public string Label { get { return "Imbuing Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Imbuing” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static ImbuingBuff()
        {
            PieAttributeRegistry.Register(new ImbuingBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Imbuing TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Imbuing” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Imbuing mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Imbuing)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Imbuing,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Imbuing for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
