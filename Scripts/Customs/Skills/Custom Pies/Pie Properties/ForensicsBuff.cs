...
{
    public sealed class ForensicsBuff : IPieAttribute
    {
        public string Id    { get { return "ForensicsBuff"; } }
        public string Label { get { return "Forensics Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Forensics” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static ForensicsBuff()
        {
            PieAttributeRegistry.Register(new ForensicsBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Forensics TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Forensics” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Forensics mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Forensics)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Forensics,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Forensics for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
