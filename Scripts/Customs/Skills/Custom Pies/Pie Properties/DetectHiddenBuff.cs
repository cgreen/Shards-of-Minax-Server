...
{
    public sealed class DetectHiddenBuff : IPieAttribute
    {
        public string Id    { get { return "DetectHiddenBuff"; } }
        public string Label { get { return "DetectHidden Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “DetectHidden” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static DetectHiddenBuff()
        {
            PieAttributeRegistry.Register(new DetectHiddenBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing DetectHidden TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “DetectHidden” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created DetectHidden mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.DetectHidden)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.DetectHidden,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} DetectHidden for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
