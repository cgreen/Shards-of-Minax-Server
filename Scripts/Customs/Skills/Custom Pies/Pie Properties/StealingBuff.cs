...
{
    public sealed class StealingBuff : IPieAttribute
    {
        public string Id    { get { return "StealingBuff"; } }
        public string Label { get { return "Stealing Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Stealing” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static StealingBuff()
        {
            PieAttributeRegistry.Register(new StealingBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Stealing TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Stealing” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Stealing mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Stealing)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Stealing,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Stealing for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
