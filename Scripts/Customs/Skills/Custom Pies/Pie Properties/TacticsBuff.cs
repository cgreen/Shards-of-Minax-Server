...
{
    public sealed class TacticsBuff : IPieAttribute
    {
        public string Id    { get { return "TacticsBuff"; } }
        public string Label { get { return "Tactics Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Tactics” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static TacticsBuff()
        {
            PieAttributeRegistry.Register(new TacticsBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Tactics TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Tactics” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Tactics mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Tactics)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Tactics,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Tactics for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
