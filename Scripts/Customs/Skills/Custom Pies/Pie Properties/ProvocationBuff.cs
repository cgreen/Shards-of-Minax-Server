...
{
    public sealed class ProvocationBuff : IPieAttribute
    {
        public string Id    { get { return "ProvocationBuff"; } }
        public string Label { get { return "Provocation Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Provocation” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static ProvocationBuff()
        {
            PieAttributeRegistry.Register(new ProvocationBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Provocation TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Provocation” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Provocation mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Provocation)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Provocation,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Provocation for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
