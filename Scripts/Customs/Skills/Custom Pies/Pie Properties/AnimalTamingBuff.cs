...
{
    public sealed class AnimalTamingBuff : IPieAttribute
    {
        public string Id    { get { return "AnimalTamingBuff"; } }
        public string Label { get { return "AnimalTaming Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “AnimalTaming” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static AnimalTamingBuff()
        {
            PieAttributeRegistry.Register(new AnimalTamingBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing AnimalTaming TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “AnimalTaming” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created AnimalTaming mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.AnimalTaming)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.AnimalTaming,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} AnimalTaming for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
