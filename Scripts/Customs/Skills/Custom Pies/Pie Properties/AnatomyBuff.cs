...
{
    public sealed class AnatomyBuff : IPieAttribute
    {
        public string Id    { get { return "AnatomyBuff"; } }
        public string Label { get { return "Anatomy Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Anatomy” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static AnatomyBuff()
        {
            PieAttributeRegistry.Register(new AnatomyBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Anatomy TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Anatomy” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Anatomy mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Anatomy)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Anatomy,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Anatomy for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
