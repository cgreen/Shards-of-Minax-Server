...
{
    public sealed class MusicianshipBuff : IPieAttribute
    {
        public string Id    { get { return "MusicianshipBuff"; } }
        public string Label { get { return "Musicianship Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Musicianship” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static MusicianshipBuff()
        {
            PieAttributeRegistry.Register(new MusicianshipBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Musicianship TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Musicianship” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Musicianship mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Musicianship)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Musicianship,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Musicianship for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
