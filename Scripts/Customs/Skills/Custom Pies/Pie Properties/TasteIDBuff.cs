...
{
    public sealed class TasteIDBuff : IPieAttribute
    {
        public string Id    { get { return "TasteIDBuff"; } }
        public string Label { get { return "TasteID Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “TasteID” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static TasteIDBuff()
        {
            PieAttributeRegistry.Register(new TasteIDBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing TasteID TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “TasteID” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created TasteID mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.TasteID)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.TasteID,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} TasteID for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
