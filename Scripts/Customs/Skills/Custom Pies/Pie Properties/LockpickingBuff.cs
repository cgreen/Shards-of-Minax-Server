...
{
    public sealed class LockpickingBuff : IPieAttribute
    {
        public string Id    { get { return "LockpickingBuff"; } }
        public string Label { get { return "Lockpicking Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Lockpicking” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static LockpickingBuff()
        {
            PieAttributeRegistry.Register(new LockpickingBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Lockpicking TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Lockpicking” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Lockpicking mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Lockpicking)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Lockpicking,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Lockpicking for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
