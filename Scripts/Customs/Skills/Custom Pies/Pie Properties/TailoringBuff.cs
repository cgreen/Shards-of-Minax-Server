...
{
    public sealed class TailoringBuff : IPieAttribute
    {
        public string Id    { get { return "TailoringBuff"; } }
        public string Label { get { return "Tailoring Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Tailoring” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static TailoringBuff()
        {
            PieAttributeRegistry.Register(new TailoringBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Tailoring TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Tailoring” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Tailoring mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Tailoring)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Tailoring,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Tailoring for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
