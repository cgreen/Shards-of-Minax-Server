...
{
    public sealed class PeacemakingBuff : IPieAttribute
    {
        public string Id    { get { return "PeacemakingBuff"; } }
        public string Label { get { return "Peacemaking Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Peacemaking” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static PeacemakingBuff()
        {
            PieAttributeRegistry.Register(new PeacemakingBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Peacemaking TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Peacemaking” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Peacemaking mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Peacemaking)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Peacemaking,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Peacemaking for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
