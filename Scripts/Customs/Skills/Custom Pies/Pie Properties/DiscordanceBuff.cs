...
{
    public sealed class DiscordanceBuff : IPieAttribute
    {
        public string Id    { get { return "DiscordanceBuff"; } }
        public string Label { get { return "Discordance Bonus"; } }
        ...
        {
            // A quick head‐cloud effect to show “Discordance” flavor
            m.FixedParticles(0x374A, 10, 15, 5036, 0x8A5, 3, EffectLayer.Head);
        }

        static DiscordanceBuff()
        {
            PieAttributeRegistry.Register(new DiscordanceBuff());
        }
        ...
        public void Apply(Mobile eater)
        {
            ...
            // ── 2) If there’s an existing Discordance TimedSkillMod from a pie, remove it ─
            // We look for any SkillMod on “Discordance” that came from a previous pie, and remove it.
            // (Assumes that all pie‐created Discordance mods are TimedSkillMod instances.)
            foreach (SkillMod sk in eater.SkillMods.ToArray())
            {
                // Cast safe: TimedSkillMod exposes a .Skill property in ServUO
                if (sk is TimedSkillMod tsm && tsm.Skill == SkillName.Discordance)
                    tsm.Remove();
            }

            // ── 3) Create and apply the new TimedSkillMod ────────────────────────
            SkillMod mod = new TimedSkillMod(
                SkillName.Discordance,   // the skill to buff
            ...
            eater.SendMessage($"You feel chemically attuned! (+{bonus} Discordance for {dur.TotalMinutes:0} min)");
            if (SoundId >= 0)
