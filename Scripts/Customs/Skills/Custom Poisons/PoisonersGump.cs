...
{
    public class PoisonersGump : Gump
    {
        private readonly Mobile _user;
        private readonly PoisonersKit _kit;
        private readonly Item[] _ingredients;            // up to 4 slots

        private const int SlotCount      = 4;
        private const int BottleLabel    = 400;          // “Craft Poison” button ID
        private const int ExamineButton  = 998;          // “Examine” button ID

        // Poisoning‐skill requirements for the four ingredient boxes (UNCHANGED)
        ...
        // Tier25  = can be used by anyone with ≥ 25% Poisoning
        // Tier50  = … ≥ 50% Poisoning (plus Tier25 items)
        // Tier75  = … ≥ 75% Poisoning (plus Tier25+Tier50)
        // Tier100 = … ≥ 100% Poisoning (plus all below)
        ...
        /// <summary>
        /// Returns the minimum Poisoning‐skill required to use this ingredient.
        /// </summary>
        public static double GetIngredientSkillReq(Item ing)
        {
            ...
            // Anything not in the above lists → requires 200% Poisoning
            return 200.0;
        }
        // ────────────────────────────────────────────────────────────────────

        public PoisonersGump(Mobile user, PoisonersKit kit, Item[] prefill = null)
            : base(50, 50)
        {
            _user        = user;
            _kit         = kit;
        ...
        private void Build()
        {
            ...
            AddLabel(180, 15, 0x44F, "Poisoner's Kit");
            ...
            AddLabel (190, 474, 0x44F, "Craft Poison");
        ...
        private string BuildEffectHtml()
        {
            ...
            var allowed  = SelectEffects(_user.Skills[SkillName.Poisoning].Value, overlaps, freq);
            ...
            return string.Join("<BR>",
                               allowed.Select(id =>
                               {
                                   var eff = PoisonEffectRegistry.Get(id);
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
            ...
            {
                int slot  = bid - 10;
                double sk = _user.Skills[SkillName.Poisoning].Value;

                // 1) Check slot‐unlock threshold (UNCHANGED)
                if (sk < SlotSkillReq[slot])
                {
                    _user.SendMessage($"You need at least {SlotSkillReq[slot]:0.0}% Poisoning to use this slot.");
                    return;
                }
            ...
            if (bid == BottleLabel)
                ...
                _user.SendGump(new PoisonersGump(_user, _kit, new Item[SlotCount]));
        ...
        private class ExamineTarget : Target
        {
            ...
            public ExamineTarget(Mobile from, PoisonersKit kit)
            ...
            protected override void OnTarget(Mobile from, object obj)
            {
                if (obj is Item item)
                {
                    ...
                    from.SendGump(new PropertyListGump(from, item));
                }
            }
        }

        // ---------------------------------------------------------------- Crafting
        private void CraftPoison()
        {
            var chosen       = _ingredients.Where(i => i != null && !i.Deleted).ToArray();
            if (chosen.Length == 0)
            {
            ...
            if (overlaps.Count == 0)
            {
                _user.SendMessage("None of the selected ingredients share a property – no poison can be brewed.");
                return;
            }

            int max = GetMaxEffects(_user.Skills[SkillName.Poisoning].Value);
            if (max == 0)
            {
                _user.SendMessage("You lack the skill to create any poison.");
                return;
            }

            var finalEffects = SelectEffects(_user.Skills[SkillName.Poisoning].Value, overlaps, freq);
            ...
            /* need a bottle */
            var bottle = _user.Backpack.FindItemByType(typeof(Bottle));
            if (bottle == null)
            {
                _user.SendMessage("You need an empty bottle.");
                return;
            }
            ...
            /* create the poison */
            var poison = new CustomPoison { Hits = Utility.RandomMinMax(2, 5) };
            poison.ClearEffects();
            foreach (var id in finalEffects)
                poison.AddEffect(id);
				
			poison.GenerateNameAndHue();	

            _user.AddToBackpack(poison);
            _user.PlaySound(0x240);
            _user.SendMessage("You have created a custom poison!");

            _user.CloseGump(typeof(PoisonersGump));
        }
        ...
        /// <summary>
        /// Exactly matches GetMaxEffects in PoisonersGump, so that PropertyListGump can call it.
        /// </summary>
        ...
        {
            var pool = PoisonEffectRegistry.All.ToList();
        ...
        private class IngredientTarget : Target
        {
            ...
            public IngredientTarget(PoisonersKit kit, int slot, Item[] ings)
                : base(2, false, TargetFlags.None)
            {
                _kit  = kit;
                _slot = slot;
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                // ── NEW: Check this item’s “tiered” Poisoning requirement ────────────
                double userPoisonSkill = from.Skills[SkillName.Poisoning].Value;
                double req             = PoisonersGump.GetIngredientSkillReq(item);

                if (userPoisonSkill < req)
                {
                    from.SendMessage($"You need {req:0}% Poisoning to use that ingredient.");
                    return;
                }
                // ────────────────────────────────────────────────────────────────────

                _ings[_slot] = item;
                from.SendGump(new PoisonersGump(from, _kit, _ings));
            }
        }
    }
}
