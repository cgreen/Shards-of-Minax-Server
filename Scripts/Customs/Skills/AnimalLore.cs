...
using Server.Engines.XmlSpawner2;   // for SocketsGump
    ...
    public class AnimalLore
    {
        ...
        private class InternalTarget : Target
        {
			private static void SendGump(Mobile from, BaseCreature c)
			{
				...
				// first: the normal AnimalLore gump
				...
				// now: if this creature has sockets, show its sockets gump too
				var sockAtt = XmlAttach.FindAttachment(c, typeof(XmlSockets)) as XmlSockets;
				if (sockAtt != null && sockAtt.NSockets > 0)
				{
					from.SendGump(new Server.Engines.XmlSpawner2.XmlSockets.SocketsGump(from, sockAtt));
				}
			}
