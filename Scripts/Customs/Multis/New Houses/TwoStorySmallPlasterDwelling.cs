...
{
    public class TwoStorySmallPlasterDwelling : BaseHouse
    {
        // Same area you used in the original code
        public static Rectangle2D[] AreaArray = new Rectangle2D[]
        {
            new Rectangle2D( -4, -4, 9, 9)
        };

        // Constructor in the "new style"
        public TwoStorySmallPlasterDwelling(Mobile owner)
            : base(0x4A, owner, 1370, 10) // <--- Adjust the lockdowns/secures as you wish
        {
			...
			Timer.DelayCall( TimeSpan.Zero, () =>
			{
				...
				var addon = new TwoStorySmallPlasterDwellingAddon();
        ...
        public TwoStorySmallPlasterDwelling(Serial serial)
            : base(serial)
        {
        }
        ...
        {
            return new TwoStorySmallPlasterDwellingDeed();
        }
