...
{
    public static class BossRoomGenerator
    {
        ...
        )
        {
            ...
            void GrowMaze(int startX, int startY)
            {
                ...
                while (cells.Count > 0)
                {
                    ...
                    if (valid.Count > 0)
                    {
                        var dir = (lastDir.HasValue && valid.Contains(lastDir.Value) && rnd.Next(100) > windingPercent)
                                  ? lastDir.Value
                                  : valid[rnd.Next(valid.Count)];
            ...
            while (openRegions.Count > 1)
            {
                ...
                foreach (var r in connectorRegions[(cx,cy)]) regs.Add(Find(r));
                int dest = regs[0];
            ...
            // --- STEP 6: widen corridors to 2 tiles --- 
            {
