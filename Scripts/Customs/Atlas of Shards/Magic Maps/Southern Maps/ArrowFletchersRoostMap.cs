// ─────────────────────────────────────────────────────────────────────────────
//  ARROW FLETCHER'S ROOST MAGIC MAP –– just its unique data + champ‐spawn override
// ────────────────────────────────────────────────────────────────────────────
...
{
    public class ArrowFletchersRoostMap : MagicMapBase
    {
        ...
        {
			new Point3D(5975, 2723, 8),
			new Point3D(3875, 1387, 35),
			new Point3D(2468, 637, 4)
        };

        // –– Its own facet ––
        public override Map DestinationFacet
            => _overrideDestinationFacet ?? Map.Map10; // Change Map.Map1 to the correct facet if needed

        // –– Allowed mini-champ types ––
        public List<MiniChampType> AllowedChampTypes => new List<MiniChampType>()
        {
            MiniChampType.Encounter7  // Replace with appropriate encounter type(s)
        };
        ...
        [Constructable]
        public ArrowFletchersRoostMap()
            : base(0x14EB, "Map to Arrow Fletcher's Roost", 1490)
        {
        }

        public ArrowFletchersRoostMap(Serial serial)
            : base(serial)
        {
        }
        ...
        /// </summary>
        public override void SpawnChallenges(Point3D centre, Map map, SpawnedContent content, Mobile owner)
        {
            // 1) Mini-champs
