/*
 * CollectionQuestScroll.cs
 *
 * A self-contained, parameter-friendly collection quest.
...
{
    // --- top‐level extension class for safe GetTypes ---
	...
	public class CollectionQuestScroll : Item
    {
        // --- ❶ default pools --------------------------------------------------

        // Anything that can be gathered / farmed can live here.
        ...
        // You already keep a *huge* reward list in UltimateAlchemyMasterReward.
        // We just reuse it so there is one authoritative reward table.
        private static readonly Type[] DefaultRewardPool =
            QuestScrollRewards.m_Rewards;             // :contentReference[oaicite:0]{index=0}
        ...
        [Constructable]
        public CollectionQuestScroll() 
            : this(null, 0, 0, new Type[0])
        {
        }


		// --- ❹a 3-parameter shortcut constructor ------------------------------

		[Constructable]
		public CollectionQuestScroll(string targetItemName, int amountNeeded, int goldReward)
			: this(
				LookupChallengeType(targetItemName),
				amountNeeded,
				goldReward,
				new Type[0]
			  )
		...
		// helper to resolve ANY Item-derived Type by name, then fallback to DefaultChallengePool
		private static Type LookupChallengeType(string name)
		{
			// 1) try to find a loaded type whose Name matches and which inherits from Item
			if (!String.IsNullOrWhiteSpace(name))
			{
				// try fully qualified lookup first (e.g. "Server.Items.Cotton")
				var fullName = $"Server.Items.{name}";
				var t = Type.GetType(fullName, throwOnError: false, ignoreCase: true);

				// if that fails, scan all loaded assemblies for a simple-name match
				if (t == null)
				{
				...
				// ensure it’s an Item subclass
				if (t != null && typeof(Item).IsAssignableFrom(t))
					return t;
			}

			// 2) otherwise, fall back to your default pool
			return PickRandom(DefaultChallengePool);
		}




        // 4-param constructor lets staff craft fully custom scrolls in one command.
        [Constructable]
        public CollectionQuestScroll(
            ...
            : base(0x14EF) // looks like a parchment scroll
        {
            ...
            _goldReward   = (customGold   > 0) ? customGold   : Utility.RandomMinMax(2_000, 15_000);
        ...
        {
            private readonly CollectionQuestScroll _scroll;

			public CollectionQuestGump(Mobile from, CollectionQuestScroll scroll) : base(75, 50)
			{
				_scroll = scroll;

				int rewardLines = _scroll._rewardTypes.Count;
				int height = 160 + (rewardLines * 25); // dynamic height based on number of rewards
        ...
        // --- ❼ targeting logic (lifted from your Alchemy version) -------------

        private class CollectionQuestTarget : Target
        {
            private readonly CollectionQuestScroll _scroll;

            public CollectionQuestTarget(CollectionQuestScroll scroll)
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                    ...
                    item.GetType() == _scroll._targetItem)
                {
                    ...
                    if (_scroll._amountCollected >= _scroll._amountNeeded)
                        ...
                        from.Target = new CollectionQuestTarget(_scroll); // chain target
                }
                else
                {
        ...
        public CollectionQuestScroll(Serial serial) : base(serial) { }
        ...
        public override void Serialize(GenericWriter writer)
        {
            ...
            writer.Write(_targetItem?.AssemblyQualifiedName ?? string.Empty);
