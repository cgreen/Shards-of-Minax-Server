...
{
    [CorpseName("the dry remains of a Orris wanderer")]
    public class OrrisRandomNPC : SosariaRandomNPC
    {
		...
		protected virtual string SpeechTraitXmlFile => 
			Path.Combine(Core.BaseDirectory, "Data", "OrrisSpeechTraits.xml");

		public List<SosariaISpeechTrait> Traits { get; private set; } = new List<SosariaISpeechTrait>();

        [Constructable]
        public OrrisRandomNPC() : base()
        {
            InitBody();
            InitOutfit();
        ...
        public OrrisRandomNPC(Serial serial) : base(serial) { }
