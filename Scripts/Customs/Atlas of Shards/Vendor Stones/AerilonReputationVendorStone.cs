...
{
    public class AerilonReputationVendorStone : StoneRewardVendor
    {
        [Constructable]
        public AerilonReputationVendorStone()
        {
            Name = "Aerilon Reputation Vendor";
            Hue  = 1161;    // choose whatever hue you like
            Movable = false;

            //
            // 1) Bind our vendor’s “coins” to the player’s Aerilon‐faction value.
            //
            ...
            //    - attachProp: the XmlMobFactions.Aerilon property
            //
            var attachType = typeof(XmlMobFactions);
            var prop       = attachType.GetProperty("Aerilon", BindingFlags.Public | BindingFlags.Instance);
            ...
            //
            // 2) Add whatever stock you like.  Players will “pay” by reducing their Aerilon faction level.
            //
			// inside AerilonReputationVendorStone()
			// Aerilon Power Scrolls Vendor Stock Definitions

			// Skills: Spellweaving, Inscription, Imbuing, Meditation, EvalInt, Mysticism
			...
			AddStock<FancyStoneWoodHomeDeed>(
				price:       8000,
				description: "An elegant two-story home blending stone and enchanted wood, favored by Aerilon scholars."
			);

			AddStock<TwoStoryStoneVillaDeed>(
				price:       15000,
				description: "A grand stone villa with arcane balconies and magical lighting, suitable for a Conclave mage."
			);

			AddStock<SpiresDeed>(
				price:       18000,
				description: "A slender, crystalline spire inspired by Aerilon’s floating towers. Unique arcane architecture."
			);

			AddStock<NeoTwoStoryBrickHouseDeed>(
				price:       9500,
				description: "A stylish, modern brick home with magical enhancements for comfort and security."
			);

			AddStock<NeoTwoStorySandstoneHouseDeed>(
				price:       10000,
				description: "A contemporary sandstone home with airy balconies, common in Aerilon’s artisan district."
			);

			AddStock<TwoStoryWoodenHomeWithPorchDeed>(
				price:       9000,
				description: "A two-story wooden residence with a wide arcane porch overlooking the clouds."
			);

			AddStock<NewCastleTowerDeed>(
				price:       35000,
				description: "A majestic, magically reinforced castle tower reaching into the skies above Aerilon."
			);

			AddStock<CamelotDeed>(
				price:       40000,
				description: "A legendary castle blending Britannian grandeur and Aerilon’s arcane advancements."
			);

			AddStock<TheCloversKeepDeed>(
				price:       45000,
				description: "A massive skyward keep, home to only the most influential Aerilon archmages."
			);
        ...
        public AerilonReputationVendorStone(Serial serial) : base(serial) { }
