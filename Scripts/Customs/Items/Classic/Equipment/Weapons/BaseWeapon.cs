    ...
    public abstract class BaseWeapon : Item, IWeapon, IFactionItem, IUsesRemaining, ICraftable, ISlayer, IDurability, ISetItem, IVvVItem, IOwnerRestricted, IResource, IArtifact, ICombatEquipment, IEngravable, IQuality
    {
        ...
        */
		...
		public static bool CheckParry(Mobile defender)
		{
			...
			{
				// Modified parry chance calculation
				double chance = (parry - bushidoNonRacial) / 800.0;
				
				if (shield != null)
				{
					chance += shield.ArmorRating / 200.0; // Factor in shield's armor rating
				}

				if (chance < 0.01)
				{
					chance = defender.Player ? 0.01 : 0.1;
				}
				else if (chance > 0.6) // Cap maximum parry chance
				{
					chance = 0.6;				}
				...
				// Modified skill check with explicit skill gain
				bool success = (chance > Utility.RandomDouble()) && defender.CheckSkill(SkillName.Parry, chance * 0.8, chance * 1.2);
				
				if (success)
				{
					// Skill gain on successful parry
					if (defender.Skills[SkillName.Parry].Base < defender.Skills[SkillName.Parry].Cap)
					{
						defender.Skills[SkillName.Parry].Base += 0.3; // Adjust the value as needed
					}
				}
				else
				{
					// Smaller skill gain on failed parry attempts
					if (defender.Skills[SkillName.Parry].Base < defender.Skills[SkillName.Parry].Cap)
					{
						defender.Skills[SkillName.Parry].Base += 0.1; // Adjust the value as needed
					}
				}
			...
			: base(itemID)
		{
			...
			XmlSockets.ConfigureRandom(
				this,                // the item you’re socketing
				20.0,                 // 2% overall chance
				0.1,                 // 0.1% 5-socket
				0.5,                 // 0.5% 4-socket
				3.0,                 // 3%   3-socket
				15.0,                // 15%  2-socket
