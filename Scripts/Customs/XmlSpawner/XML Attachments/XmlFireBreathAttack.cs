...
namespace Server.Engines.XmlSpawner2
{
    public class XmlFireBreathAttack : XmlAttachment
    {
        ...
        {
            return String.Format("Breath Attack: {0} damage over {1} tiles every {2} seconds.", m_BaseDamage, m_Range, m_Delay.TotalSeconds);
        }
        ...
        public override void OnWeaponHit(Mobile attacker, Mobile defender, BaseWeapon weapon, int damageGiven)
        {
            if (DateTime.Now >= m_EndTime)
            {
                ...
                m_EndTime = DateTime.Now + m_Delay;
            }
        }
		...
		private void SpawnFireField(Point3D location, Map map, Mobile attacker)
		{
			...
			FireFieldSpell.FireFieldItem fireField = new FireFieldSpell.FireFieldItem(0x398C, validLocation, attacker, map, TimeSpan.FromSeconds(30), 20);
